...
def best_first_graph_search_tracking(problem, f, display=False):
    """ 
        Modifications for Q2
        2) Number of tiles moved for solution
        3) Total number of nodes that were REMOVED from frontier
    """
    f = memoize(f, 'f')
    node = Node(problem.initial)
    frontier = PriorityQueue('min', f)
    ...
    while frontier:
        node = frontier.pop()
        """ Q2 Modification (3) """
        countNodesRemoved = countNodesRemoved + 1
        ...
        if problem.goal_test(node.state):
            ...
            """ Q2 Modification (3) """
            print("Total number of nodes that were REMOVED from frontier is:", countNodesRemoved)
            
            """ Q2 Modification (2) """
            print("Number of tiles moved for solution :", node.path_cost)
...
def astar_search(problem, h=None, display=False):
    ...
    returnNode = best_first_graph_search_tracking(problem, lambda n: n.path_cost + h(n), display)

    """ Q2 Modification (1) -Taken from Assignment 1 Documentation"""
    elapsed_time = time.time() - start_time
    print(f'Elapsed time for default Misplaced Tile search (in seconds): {elapsed_time}s')
