...
#Qsort code taken from https://www.geeksforgeeks.org/python-program-for-quicksort/
    ...
    def difference(self,state):
        ...
        for x in range(9):
            if self.goal[x] != state[x]:
                accum += 1
        return accum
    def make_rand_8puzzle(self,current):
            for x in range (10000):
                    prev = current
                    actions = solver.actions(current)
                    rand = random.randint(0,len(actions)-1)
                    current = solver.result(prev,actions[rand])
            return current
...
current = init = solver.make_rand_8puzzle(current)
solver = EightPuzzle(init)
solver = EightPuzzle(init)
...
accum = 0
memo = []
if(not (solver.check_solvability(init))):
    print("Not possible")
    error = True
    exit
else:
    ...
    while (not(solver.goal_test(current))):
        ...
        for x in range(length):
            test = solver.result(prev,actions[x])
